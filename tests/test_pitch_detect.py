...
def test_pure_tone_detected_within_10_cents(freq):
    tone = make_tone(freq)
    detected, confidence = detect_pitch(tone, SAMPLE_RATE)
...
def test_silence_returns_none():
    ...
    detected, confidence = detect_pitch(silence, SAMPLE_RATE)
...
def test_white_noise_low_confidence_or_none():
    ...
    detected, confidence = detect_pitch(noise, SAMPLE_RATE)
