...
def render_audio(track, sample_rate=None, drum_level=0.5):
    ...
    from notecolor.audio.playback import render_offline
...
def render_drums(track, sample_rate=None):
    ...
    sample_rate = sample_rate or config.PLAYBACK_SAMPLE_RATE
    # The drum models live in the repo's own test harness rather than in the
    # package (nothing new is synthesized that this project did not already
    # have -- see this module's docstring), so this is a checkout-only tool:
    # walk up from the package to whichever ancestor holds `scripts/`, rather
    # than assuming a fixed depth that the next layout change would break.
    relative = os.path.join("scripts", "acoustic_pipeline_test.py")
    ...
    while True:
        ...
        if parent == directory:
            raise FileNotFoundError(
                f"Could not find {relative} in any parent of this package. "
                "Drum rendering reuses the repo's acoustic test harness and so "
                "only works from a source checkout, not an installed wheel."
            )
        directory = parent
    spec = importlib.util.spec_from_file_location("_acoustic_synth", candidate)
