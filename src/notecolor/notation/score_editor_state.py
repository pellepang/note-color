...
def note_index_at_row(column, row):
    """Index into `column.notes` of whichever note (if any) sits at `row`, or
    None.

    Lives here rather than in the editor's display module (its home until
    ticket #146) because it is a property of a column, not of how a column is
    drawn: it answers "which note is on this staff row", which the audition
    logic and the DAW's own editors need just as much as the terminal
    renderer does. `score_editor_display` re-exports it for its own callers.
    """
