...
from notecolor.analysis.duration_tracker import DEFAULT_DURATION_CLASS, beats_for_duration_class
from notecolor.notation.score_editor_state import note_index_at_row

#: Modifier bits that do not make a keypress deliberate. Caps Lock and Num
#: Lock are states, not gestures -- holding neither is a decision the player
#: made -- so a note key still sounds with either engaged, while any real
#: modifier falls through to normal keybind dispatch (`m` is a B, `Shift`+`M`
#: is the metronome).
#:
#: Stated here in the core's own terms rather than imported from the
#: terminal's kitty-protocol parser: the core must not know which front-end is
#: reporting modifiers, since the DAW will report Qt's. The values are pinned
#: against `kitty_keys` by a test, the same cross-check convention this repo
#: already uses for `parse_key_event()` and the synth-layout seam.
MOD_CAPS_LOCK = 0b1000000
MOD_NUM_LOCK = 0b10000000
LOCK_MODIFIERS = MOD_CAPS_LOCK | MOD_NUM_LOCK
