...
This doc sat uncommitted for a while, and most of its Part 2 feature
ideas (and the Concept E infrastructure Part 1 recommended adopting
first) have since actually shipped — recording that here rather than
leaving the doc's own "what would this cost to build" framing looking
like unstarted work:

- **Concept E (JSONL session log): adopted**, essentially as proposed —
  `session_recorder.py`'s `SessionRecorder` (the `S` keybind, opt-in,
  off by default) writes one JSON line per finalized note to
  `session_log_<timestamp>.jsonl`; `session_player.py` reads it back.
- **Concept A (ABC as source of truth): adopted in exactly the scoped
  way this doc recommended** — *not* a rewrite of `TabDisplay`'s live
  data model, but an additive export path. `abc_export.py`'s
  `note_events_to_abc()`/`from_transcription_result()`/
  `from_session_log()` hand-roll ABC serialization (confirmed, same as
  this doc's own retrospective sibling doc found, that `music21` can
  read but not write ABC) and are wired into `virtualnote transcribe
  --export-abc`. The live staff renderer itself is untouched, as
  recommended.
- **Feature 1 (session recording + playback): shipped** —
  `virtualnote replay <file> [--speed N]` (`main.run_replay_session()`),
  built directly on Concept E's log as this doc anticipated.
- **Feature 2 (export): partially shipped.** ABC export is done
  (`abc_export.py`, above). MusicXML export actually predates this doc
  (`score_writer.py`, issue #65, committed 2026-08-21) — this doc's own
  "lowest priority of the three" ranking for MusicXML was already
  overtaken by the time it was written. Standard MIDI file export is
  **not** built — still open, and still a reasonable "small-to-medium,
  no new dependency" scoped idea per this doc's own sizing.
- **Feature 3 (practice mode / play-along scoring): not built.** Still
  open as described.
- **Feature 4 (historical play stats): shipped** — `stats_display.py`,
  `virtualnote`'s Stats menu entry, aggregating `session_log_*.jsonl`
  files essentially as this doc describes (total practice time,
  most-played notes, sessions-by-date).
- **Feature 5 (multi-track/multi-instrument view): not built.** Still
  open, still the largest-scope idea on the list per this doc's own
  sizing.
- **Feature 6 (loop/section markers): shipped** — the tab view's
  freeze-mode-only `[`/`]` marks (`mark_range_start`/`mark_range_end`),
  scoping a subsequent `R`-key reanalysis to the marked range, built on
  the existing freeze/scrollback/reanalysis machinery essentially as
  this doc's "almost entirely wiring" framing anticipated.

Concepts B, C, D (piano-roll, tracker grid, braille cells) remain
unbuilt as *rendering* styles — consistent with this doc's own
recommendation not to pursue them as primary — but the six-prototype
retrospective in `project-retrospective-and-alternatives.md` (this same
research pass) did build B and C as real runnable prototypes to check
that recommendation against actual code, not just theory; see that
doc's Part 3.
