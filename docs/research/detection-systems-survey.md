...
This doc sat uncommitted for a while, and all four of §3's near-term
recommendations have since actually been executed — recording that here
rather than leaving the "worth doing now or soon" framing below looking
like an open TODO list it no longer is:

1. **Port `hfc`/`complex`-domain onset novelty (recommendation 1): done.**
   `onset_detect.py` now has `hfc_novelty()`/`complex_domain_novelty()`,
   validated against real `--source loopback` audio
   (`prototypes/onset-novelty-hfc/`). Issue #78.
2. **Tempo-octave-lock mitigation (recommendation 2): done.**
   `tempo_tracker.py`'s `TempoTracker._resolve_octave_lock()` checks
   `acf[2*best_lag]`/`acf[best_lag//2]` against the existing confidence
   gate, essentially as sketched below. Issue #79.
3. **NNLS-based chroma-fold prototype (recommendation 3): done, at
   prototype stage.** `prototypes/nnls-chroma/` exists; per this survey's
   own §4 conflicts section, it was framed as "worth trying for chord
   accuracy generally," not as a fix for the still-open harmonic-collision
   case — that framing held, this is a prototype, not yet a change to
   `chroma.fold()` itself. Issue #81.
4. **Time-boxed source read of SwiftF0's inference code (recommendation
   4): done, and settled in the negative.** `docs/research/
   swiftf0-source-verification.md` (committed) confirms SwiftF0 requires
   `onnxruntime` and ships a bundled `model.onnx` — the "claims
   `numpy`-only inference, not independently confirmed" note against
   SwiftF0 in the comparison table below (§2) is now resolved: the claim
   is **false**. See that doc's own summary, or the inline note added at
   the SwiftF0 table row below. Issue #80 (open, source-verification
   scope only — Pi-hardware latency/accuracy benchmarking is still
   unstarted, needs real Pi hardware).

Recommendations 5-7 (mid/long-term, gated on a concrete future need) are
unaffected by this update and remain as originally written.
...
| **SwiftF0** (2025, `pip install swift-f0`) | Mono pitch, octave-error + noise robustness | Best-in-class across CREPE/pYIN/Praat/PENN on the one paper that benchmarks all of them: 94.07% clean / 91.80% noisy HM, 96.75%/93.52% octave accuracy | 132.6ms/5s clip on **desktop** (42x faster than CREPE) — **no Pi number exists anywhere**, genuinely unmeasured | Small (95,842-param CNN); claims `numpy`-only inference, **not independently confirmed by reading its source** — **[Status update, 2026-09-02: independently confirmed, and refuted — see `docs/research/swiftf0-source-verification.md` (issue #80). SwiftF0 hard-requires `onnxruntime` and ships a bundled ONNX model; there is no NumPy-only forward-pass path. It is in the same dependency class as basic-pitch below, not a lighter one.]** | Would replace/augment `pitch_detect.detect_pitch()` live — the one live-path candidate that doesn't structurally fail the constraints on paper — **[see status update: this integration point claim is now qualified by the same ONNX-runtime dependency weight basic-pitch already carries; treat SwiftF0 as belonging in this table's basic-pitch tier (offline/desktop-optional), not as a runtime-dependency-free live candidate]** |
...
5. **basic-pitch (ONNX) as an optional `virtualnote transcribe` backend,
   ...
   complaint, not preemptively. *(Status update, 2026-09-02: SwiftF0's
   now-confirmed ONNX dependency, see above, puts it in this exact same
   tier as basic-pitch — an optional offline backend evaluated alongside
   it, per `swiftf0-source-verification.md`'s own conclusion.)*
...
6. **A genuinely hardware-tiered live neural pitch backend** (SwiftF0
   ...
   *(Status update, 2026-09-02: the parenthetical's "if SwiftF0's
   no-runtime claim doesn't hold up" branch is now the confirmed case —
   SwiftF0 does need a runtime, `onnxruntime`, same as SPICE/CREPE-tiny's
   TFLite/PyTorch. This doesn't newly justify pursuing this item; it just
   removes SwiftF0's one hypothetical exemption from the "runtime
   dependency" cost column.)*
