...
| `score_properties_display.py` | (issue #98, per #90) The Score properties screen — opened by `score_properties` (`t`), closed by `score_properties_exit`. Three Reels (`PROPERTY_SLOTS`): time signature (`spin_time_signature()`, steps `TIME_SIGNATURE_OPTIONS`' small fixed set), key signature (`spin_key_fifths()`, +/-1 around the circle of fifths, clamped at +/-7 matching `music21.key.KeySignature`'s valid range), tempo (`spin_tempo()`, +/- `TEMPO_STEP_BPM` clamped into `[TEMPO_MIN_BPM, TEMPO_MAX_BPM]`, same clamp-not-wrap convention `settings_display.parse_numeric_input()` uses). Unlike the Chord builder, mutates the real `EditorScore` passed in directly as each reel spins — no working-copy/commit step, since three independent scalar fields have no "which notes should this become" staging ambiguity to justify one. `render()` is smoke-tested manually only. |
...
| Space | `note_toggle` | place/remove a note at the cursor (refuses to empty a column to zero notes — use `r` for that) |
| `+`/`-` | `transpose_up`/`transpose_down` | shift the note under the cursor a semitone |
| `,`/`.` | `duration_shorten`/`duration_lengthen` | step the column's duration through the standard note-value set |
| `r` | `clear_to_rest` | empty the column's notes outright (the only way to create a Rest) |
...
| `t` | `score_properties` | open the **Score properties** screen |
| `\|`/`H` | (global) | back-to-menu / legend toggle, unaffected |

Status line shows `saved=yes/no`; quitting (`|`/Ctrl+C) while `saved=no`
needs a second confirming press of the same key before actually
discarding changes — the one editor view in this app where quitting can
lose real work, unlike every other terminal view's purely ephemeral
render state.
...
(root/quality/3rd/5th/7th): Left/Right switches the focused reel, Up/Down
spins it, and typing jumps it (a natural letter on ROOT, e.g. `F`, jumps
straight there — `#`/`b` immediately after nudges it a semitone; a
mnemonic like `m7`/`dim`/`b5`/`sus4` on the other reels auto-commits the
instant it's unambiguous). Every spin/typeahead applies live to the
screen's own working chord; `b` commits it into the real column on exit.
Root is ordered around the circle of fifths, same theming every other
fifths-scheme view in this app already uses.

The **Score properties** screen (`t` to open, `score_properties_exit` —
default `b` — to leave) has three Reels the same way: time signature
(steps a small set of common signatures), key signature (steps the
circle-of-fifths position ±1), tempo (steps by a fixed BPM increment,
clamped 20–300). Applies directly to the loaded score as each reel spins.
...
- `[keybinds]` — remap any of the nine terminal hotkeys (`source_toggle`,
  ...
  `mark_range_start`, `mark_range_end`) plus the score editor's sixteen
  (issue #98 — `note_toggle`, `transpose_up`, `transpose_down`,
  ...
  `score_properties_exit`) to a different single character, e.g.
  `source_toggle = "x"`. `rhythm_reanalysis` (default `"r"`) is the
...
- The score editor's `undo`/`redo` keybinds (default `"u"`/`"U"`) are
  ...
  are `chord_builder_exit`/`score_properties_exit` (default `"b"`),
  exact-case for a different reason: the Chord builder's ROOT reel needs
  to type the actual letter 'B', which collides with 'b' as a plain
  case-insensitive exit key. See docs/DECISIONS.md for both, and for why
