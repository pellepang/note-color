...
`run_session()` looks up which run_* function to call, and how to call it,
through `main.VIEWS` (architecture-modernization-plan.md §3.3's display
dispatch table) rather than a hand-written if/elif chain — the same table
also drives `menu_display.TOOLS` (the menu's tool list) and
`virtualnote.build_parser()`'s `fill`/`wheel`/`tab`/`gui` subparsers, so
all three no longer risk drifting out of sync about which views exist —
...
- `analysis_loop()` calls its two detection functions through
  ...
  design against.
- `run_session()`'s string-keyed if/elif chain over `"fill"`/`"wheel"`/
  `"tab"`/`"gui"` is replaced by `main.VIEWS`, a dict built once and read
  by three call sites (`run_session()` itself, `menu_display.TOOLS`,
  `virtualnote.build_parser()`) instead of each hand-writing its own copy
  of "which views exist" (`docs/research/architecture-modernization-
  plan.md`'s §3.3). Deliberately narrower than it might look: each
  entry's `dispatch` callable (`main._dispatch_fill`/`_dispatch_wheel`/
  `_dispatch_tab`/`_dispatch_gui`) is the one place that adapts
  `run_session()`'s common argument bundle to that view's own real
  signature — `run_terminal_fill`/`run_terminal_wheel`/`run_terminal_tab`/
  `run_gui` themselves keep their existing, genuinely different parameter
  lists unchanged (`run_terminal_tab` alone needs `scroll_mode`/
  `dump_file`/`reanalysis_buffer`/`time_signature` the other three don't).
  This is *dispatch*-table scope only, exactly as the plan doc scopes it —
  no shared `render()` Protocol was introduced across `TerminalDisplay`/
  `WheelDisplay`/`TabDisplay`/`Display`, since their real state differs
  too much (`TabDisplay.render()` alone takes 7 params with no fill/wheel
  equivalent) for a uniform signature to be anything but a `**kwargs`/
  `RenderContext` object hiding the same differences one level down — the
  plan doc calls that out explicitly as the one way this proposal could
  become the roadmap's biggest risk. `transcribe`/`replay` are
  deliberately not `VIEWS` entries even though `virtualnote.build_parser()`
  also builds subparsers for them — neither is a `SessionState`-driven
  live view (see Architecture); folding them in would misrepresent them
  as another `run_session()`-launchable tool.
