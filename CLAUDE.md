...
| `session_score.py` | (issue #89, map #57's live-score-saving child ticket) `from_session_log(events)` — a `session_player.load_events()`-shaped event list in, a `batch_transcribe.TranscriptionResult` out, so `score_writer.write_score()` (unchanged) can write a MusicXML file straight from a recorded session log. No non-causal duration/tempo *re*-refinement is attempted — a session log only has each note's already-finalized `duration_seconds`/`t`/`bpm_estimate`, not the raw per-hop magnitude/onset arrays `DurationTracker.finalize_noncausal()`/`rhythm_reanalysis.py` would need to redo that work (issue #89's investigation confirmed this gap is unreconstructable after the fact, but immaterial to score-writing itself, which only ever reads already-finalized fields). `hop_seconds=1.0` and `onset_hop=t`/`duration_hops=duration_seconds` reduce `score_writer.py`'s hop-based arithmetic to using real seconds directly; `bpm=` the median of the log's non-null `bpm_estimate` values (`None` if none logged, hitting `write_score()`'s existing `None`-bpm fallback); `chroma_histogram=` a coarse duration-weighted-by-pitch-class proxy (cruder than batch's harmonic-weighted `chroma.fold()` sum, but `guess_key_signature()`'s confidence threshold already falls back to "no key signature" rather than propagate a bad guess). `result.mono_notes` is always `[]` — `write_score()` never reads it, and a session log's mono-note vs. chord-tone events aren't reliably separable after the fact anyway. Mirrors `abc_export.py`'s own `from_session_log()` — a small adapter module per consumer of the log, not a shared "session log means X" module. Wired into `virtualnote replay --write-score [PATH]`, same `None`/`""`/explicit-path convention `transcribe --write-score` already uses. |
| `playback.py` | (map #24, decision #32) Score playback: NumPy oscillator+ADSR synth reusing `sounddevice.OutputStream` the way `audio_capture.py` reuses `InputStream` — no new dependency. `note_frequency()` (standard MIDI tuning, A4=440Hz), `_adsr_envelope()` (linear-segment attack/decay/sustain/release, scaled down proportionally for a note shorter than attack+decay+release so it still reaches zero by its own end rather than clipping mid-release), `synthesize_note()` (a small fixed harmonic-stack waveform — fundamental+2nd+3rd partials, `config.PLAYBACK_HARMONIC_WEIGHTS` — shaped by the envelope). Two first-class, independently-integrated modes per decision #32 ("both, not a single pick"): `render_offline()`/`play_offline()` (whole-buffer pre-render, additively mixed + `tanh` soft-clipped, sample-accurate by construction — used by `virtualnote transcribe --play`, since a whole `TranscriptionResult` already exists before playback starts) and `LiveScheduler` (`trigger_note()` synthesizes and hands a note to a lock-protected list of active voices an `OutputStream` callback mixes in as pulled, bounded onset latency = one callback block, ~11ms at the defaults — used by `virtualnote replay --play`, triggered directly off `run_replay_session()`'s own already-paced loop rather than running a second independent clock; `schedule_note(..., delay_seconds=...)` exists for a future caller that needs the scheduler to own timing itself, via a dedicated thread sleeping in short bounded increments toward each due event rather than one naive full-duration sleep per note — no current caller uses it). Never imported by `analysis_loop()`/`SessionState` — playback is strictly opt-in offline/replay-time, same isolation convention as `librosa`/`music21`, though `sounddevice` itself is already a live-path dependency so this is about usage, not import cost. |
...
virtualnote replay session.jsonl --write-score out.musicxml           # same, plus a MusicXML score built from the log
...
`virtualnote replay <file> --write-score [PATH] [--time-signature N/D]`
(issue #89, map #57's live-score-saving child ticket) writes a MusicXML
score built straight from the session log, same `None`/bare-flag-default-
path/explicit-path convention `transcribe --write-score` already uses
(default `score_<timestamp>.musicxml` next to `main.py`). Reuses
`score_writer.write_score()` unmodified, fed by a small adapter
(`session_score.from_session_log()`) that treats each logged note's own
already-finalized onset/duration as final rather than attempting any
non-causal rhythm *re*-refinement — a session log never recorded the raw
per-hop data that refinement would need (see that module's Files-table
entry and issue #89 for the full investigation). `--time-signature`
(default `4/4`) only affects the exported score's `<time>` field, not
the on-screen replay itself (a session log never records barlines, v1
scope — see `session_recorder.py`'s entry above). Works alongside
`--play`/`--speed`/`--dump-file`; written once, after the replay loop
exits (normally or via Ctrl+C), from the log's full event list, not just
whatever was visibly replayed.
...
- `session_score.from_session_log()` (issue #89) deliberately skips
  non-causal duration/tempo *re*-refinement rather than attempting a
  partial reconstruction of it — `session_recorder.py`'s JSONL schema
  logs only already-finalized note events (`t`/`duration_seconds`/
  `bpm_estimate`/`chord_name`), never the raw per-hop magnitude/onset
  arrays `DurationTracker.finalize_noncausal()`/`rhythm_reanalysis.py`
  need to redo that work, and there's no way to fake one from the other
  after the fact without just reproducing the same duration the log
  already has. Confirmed (not assumed) small in practice:
  `score_writer.write_score()` itself only ever reads already-finalized
  `onset_hop`/`duration_hops`/`pitch_class`/`octave`/`chord_name`/`bpm`/
  `chroma_histogram` fields, every one of which reduces to arithmetic
  already present in the log (`hop_seconds=1.0` makes `onset_hop=t`/
  `duration_hops=duration_seconds` exact, not an approximation). A
  session-derived score therefore carries the same "provisional, not
  chased further without a concrete complaint" posture this codebase
  already applies to causal rhythm data elsewhere, rather than a new
  kind of inaccuracy specific to this feature.
- `session_score.py` is its own small module, not folded into
  `session_player.py` (which stays a pure log-reading/grouping module
  with no `batch_transcribe`/`score_writer` knowledge) or into
  `score_writer.py` itself (which stays a pure music21-consuming writer
  with no session-log-schema knowledge) — mirrors `abc_export.py`'s own
  `from_session_log()`, one small adapter module per consumer of the log
  rather than a shared "session log means X" module. `score_writer.py`
  needed zero changes (issue #89's resolution) — the adapter's output is
  simply shaped to match `batch_transcribe.TranscriptionResult` exactly.
...
- `playback.py` implements both playback modes decision #32 committed to
  (offline pre-render and live per-event scheduling) as two genuinely
  different code paths, not one mode simulating the other, because they
  serve two different real callers with different natural shapes:
  `virtualnote transcribe --play` already has a complete
  `TranscriptionResult` in hand before playback starts, so
  `render_offline()` pre-rendering the whole thing to one NumPy buffer up
  front is strictly simpler and free of scheduling jitter by
  ...
  loop, calling `LiveScheduler.trigger_note()` the instant each column is
  pushed -- deliberately *not* pre-rendering the whole session up front
  (which would mean buffering a recording of unbounded length before
  playback could start at all) and deliberately *not* giving
  `LiveScheduler` its own independent timing clock for this caller (which
  would risk the audio and the on-screen columns drifting apart against
  each other over a long replay). `LiveScheduler.schedule_note()` (a
  dedicated-thread, `delay_seconds`-based API) exists for a hypothetical
  future caller that has no such already-paced loop of its own to piggy-
  back on, but nothing in this codebase calls it yet.
- `LiveScheduler`'s active-voice list is guarded by an explicit
  ...
  a single atomic full-snapshot read) per access, while `LiveScheduler`'s
  audio callback does a read-modify-write across the *whole* active-voice
  list every block (mix each voice's samples, advance its position,
  rebuild the list minus any that just finished) -- not a single atomic
  bytecode op, so a real lock is the correct call here rather than
  reaching for the same GIL-based argument.
