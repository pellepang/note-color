"""Kitty keyboard protocol support (wayfinder map #99, ticket #118).

Pure logic only: negotiation byte sequences, capability detection with a
safe fallback, the CSI event-encoding parser (press / repeat / release,
modifiers, several simultaneously-held keys), and the two held-note
policies a QWERTY piano needs -- the real one (kitty releases) and the
degraded one (fixed-duration notes, for a terminal that reports none).

Nothing in here does I/O or opens a terminal. `main.RawKeys` is the thin
I/O layer on top, and is the only module that calls into this one today;
everything here is unit-tested in `tests/test_kitty_keys.py` against
plain byte strings, per this repo's "pure logic unit-tested, real
terminal I/O smoke-tested" convention.

Why this exists: an ordinary terminal reports only key *presses*, so a
held key is indistinguishable from a key struck repeatedly and a
sustaining instrument has no note-off to work with. The kitty keyboard
protocol reports releases, which is what makes held-note QWERTY playing
possible at all. Landed from the verified prototype in issue #101, which
the project owner confirmed live in real kitty (held keys sustain; three
simultaneous keys release independently, out of order).
...
# actually installed -- which is what lets `legacy_token()` below hand
# existing RawKeys callers the same plain characters they get today.
SYNTH_FLAGS = (
...
#: DECSET 1004 -- ask the terminal to report focus in/out as ``CSI I`` /
#: ``CSI O``. Not part of the kitty keyboard protocol at all (it predates
#: it and is widely supported), but it is the missing half of held-note
#: safety: if the window loses focus mid-hold, the release event for a key
#: still physically down is delivered to whoever has focus now, never to
#: us, and the note would hang forever. `main.RawKeys` enables this
#: alongside the keyboard mode and synthesises releases on focus-out.
FOCUS_TRACKING_ON = b"\x1b[?1004h"
FOCUS_TRACKING_OFF = b"\x1b[?1004l"

#: CSI final bytes for the two focus reports.
FOCUS_IN_FINAL = "I"
FOCUS_OUT_FINAL = "O"
...
def parse_key_event(param_bytes, final_byte):
    ...
    the strict generalisation of it, and `tests/test_kitty_keys.py`
    cross-checks the two against each other for every arrow form rather
    than trusting that by inspection.
